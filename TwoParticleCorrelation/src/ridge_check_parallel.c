    ...
    )
{
    ...
    for (Int_t i=0;i<nevent;i++) {
        ...
        if( nTrk < 0) continue;
        Int_t histNtrk = s.histNtrk(nTrk);
	
        h_eff->Fill(nTrk/data.nParticle);
        h_Aj->Fill(s.fillAj);
        multiplicity->Fill(nTrk);
        nSignalEvts[histNtrk] += 1;
        ...
        for ( Int_t j=0;j<data.nParticle;j++ )
        {
            ...
            Int_t histPt1 = s.histPt(data.getPt(j));
            Int_t histEta1 = s.histEta(data.getEta(j));
            if(histPt1 == -1 || histEta1 == -1) continue; 
            ...
            for ( Int_t k=j+1;k<data.nParticle;k++ )
            {
                ...
                Int_t histPt2 = s.histPt(data.getPt(k));
                Int_t histEta2 = s.histEta(data.getEta(k));
                if(histPt1 != histPt2 || histEta1 != histEta2) continue; 
        ...
        for (Int_t nMix = 0; nMix<s.bkgrd_runs; nMix++)
        {
            ...
            for ( Int_t j=0;j<data.nParticle;j++ )
            {
                ...
                // Decide which pt and eta range to fill
                Int_t histPt_bkg1 = s.histPt(data.getPt(j));
                Int_t histEta_bkg1 = s.histEta(data.getEta(j));
                if(histPt_bkg1 == -1 || histEta_bkg1 == -1) continue; 
                ...
                for ( Int_t k=0;k<mix.nParticle;k++ )
                {
                    if(!s.ridge_trackSelection(data.nTPC[k],data.getTheta(k),data.getPmag(k), data.d0[k], data.z0[k], data.pwflag[k])) continue;
                    // Check if the second particle is in the same range of pt and eta    
                    Int_t histPt_bkg2 = s.histPt(data.getPt(k));
                    Int_t histEta_bkg2 = s.histEta(data.getEta(k));
                    if(histPt_bkg1 != histPt_bkg2 || histEta_bkg1 != histEta_bkg2) continue;
