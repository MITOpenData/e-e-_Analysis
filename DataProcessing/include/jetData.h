...
void jetData::SetStatusAndAddressRead(TTree* inTree_p, std::vector<std::string> inList)
{
  ...
  if(varIsGood[5]) inTree_p->SetBranchAddress("jtN", jtN);
  if(varIsGood[6]) inTree_p->SetBranchAddress("jtNPW", jtNPW);
  if(varIsGood[7]) inTree_p->SetBranchAddress("jtptFracPW", jtptFracPW);
