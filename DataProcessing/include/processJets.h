...
void processJets(std::vector<fastjet::PseudoJet> p, fastjet::JetDefinition jDef, fastjet::JetDefinition jDefReclust, jetData *d, const double ptCut, const double rParam, const int nExclJet)
{
  ...
  if(p.size() > 0){
    ...
    if(rParam > 0) jets = fastjet::sorted_by_pt(cs.inclusive_jets());
    else jets = fastjet::sorted_by_pt(cs.exclusive_jets(nExclJet));

    for(unsigned int i = 0; i < jets.size(); ++i){
      if(jets.at(i).pt() < ptCut && rParam > 0) break; //Arbitrarily low cut on jets, removes spike at phi zero when things become ill defined
	...
	d->jtNPW[d->nref][jetConst.at(k).user_index()]++;
      ...
      for(int j = 0; j < 6; ++j){
	if(subJets.at(j).size() == 0) d->jtptFracPW[d->nref][j] = 0.;
	else{
	  fastjet::ClusterSequence csSub(subJets.at(j), jDefReclust);
	  
	  std::vector<fastjet::PseudoJet> constTot;
	  if(rParam > 0) constTot = fastjet::sorted_by_pt(csSub.inclusive_jets());
	  else constTot = fastjet::sorted_by_pt(csSub.exclusive_jets(1));
