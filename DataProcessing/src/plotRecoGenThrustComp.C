...
int plotRecoGenThrustComp(const std::string inFileName)
{
  ...
  for(Int_t hI = 0; hI < nHist; ++hI){
    TH1F* hist1_p = (TH1F*)inFile_p->Get(("reco" + hist[hI] + "WrtRecoThr_h").c_str());
    TH1F* hist2_p = (TH1F*)inFile_p->Get(("reco" + hist[hI] + "WrtGenThr_h").c_str());
    ...
    leg_p->AddEntry(hist1_p, "Reco. Thrust", "P L");
    leg_p->AddEntry(hist2_p, "Gen. Thrust", "P L");
    ...
    hist1_p->SetMaximum(TMath::Max(hist1_p->GetMaximum(), hist2_p->GetMaximum())*1.5);
    ...
    canv_p->SaveAs(("pdfDir/recoGenThrustComp_" + hist[hI] + "_" + std::to_string(date->GetDate()) + ".pdf").c_str());
