...
float thrust(float x[]){
  ...
  */
  ...
  //float x[nevent_process];
   ...
   //x[i] = hprojection[i]->GetXaxis()->GetBinCenter(binmax);
   x[i] = theta_max;
...
void scan_thrust(){
  ...
  Int_t nevent = (Int_t)t1->GetEntries();
  int nevent_process = 150;
  //int nentries=newtree->GetEntries();
  //if the number of events (nentries is a variable) is unknown, 
  //put x[nentries] instead and uncomment the line starting at memset
  ...
  for( int i=0; i < nevent_process; i++){ //replace i<number by i<nentries if nentries is not known
     t1->GetEntry(i);
     tAngle = x[i];
     newtree->Fill();
  }
