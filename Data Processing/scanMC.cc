...
{
    std::vector<std::string> fileList = returnFileList("/Users/anthony/Desktop/GGBB",".aleph");
    for (int i =0; i <4;i++){cout<<fileList[i]<<endl;}
    FILE *fp=fopen(Form("%s",infile1.Data()),"r");
    
    float _process,_px,_py,_pz,_m,_charge,_pwflag,_pid;
    
    TFile *hf = new TFile(Form("%s.root",infile1.Data()), "RECREATE" );
