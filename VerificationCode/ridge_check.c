...
void analysis(int isBelle=0,int isThrust=1, int maxevt=0,int mult_low=0,int mult_high=10,int nbin=40,bool verbose=0,int num_runs=20){
  ...
  TH2F *h_ratio = new TH2F ( "h_ratio", "eta-phi of all particles ", nbin, -detaRange, detaRange,nbin, -3.1416/2.,3.1416*1.5);
    ...
    //TH2F *h_2D_theta = new TH2F ( "h_2D_theta", "theta-phi of all particles ",nbin, -dthetaRange, dthetaRange,nbin, -3.1416/2., 3.1416*1.5);
    //TH2F *h_2Dmix_theta = new TH2F ( "h_2Dmix_theta", "theta-phi of all particles ",nbin, -dthetaRange, dthetaRange,nbin, -3.1416/2., 3.1416*1.5);
    //TH2F *h_ratio_theta = new TH2F ( "h_ratio_theta", "theta-phi of all particles ", nbin, -dthetaRange, dthetaRange,nbin, -3.1416/2.,3.1416*1.5);
  ...
  if( maxevt>0 && maxevt<nevent ) nevent_process = maxevt;  
    ...
    for ( int j=0;j<nparticles;j++ ) {
      ...
      for ( int k=j+1;k<nparticles;k++ ) {
        ...
        //cout<<eta1-eta2<<" "<<dphi(phi1,phi2)<<endl;
        if (N!=0)
          {
        ...
        h_2D->Fill(eta1-eta2,dphi(phi2,phi1),1./N);
          //cout<<"H_2D Fill 2 "<<h_2D->GetBinContent(eta1-eta2,dphi(phi2,phi1))<<endl;
        h_2D->Fill(eta2-eta1,dphi(phi1,phi2),1./N);
          //cout<<"H_2D Fill 3 "<<h_2D->GetBinContent(eta2-eta1,dphi(phi1,phi2))<<endl;
        h_2D->Fill(eta2-eta1,dphi(phi2,phi1),1./N);
          //cout<<"H_2D Fill 4 "<<h_2D->GetBinContent(eta2-eta1,dphi(phi2,phi1))<<endl;
              
      //h_2D_theta->Fill(theta1-theta2,dphi(phi1,phi2),1./N);
      
      //h_2D_theta->Fill(theta1-theta2,dphi(phi2,phi1),1./N);
      //cout<<"H_2D Fill 2 "<<h_2D->GetBinContent(eta1-eta2,dphi(phi2,phi1))<<endl;
      //h_2D_theta->Fill(theta2-theta1,dphi(phi1,phi2),1./N);
      //cout<<"H_2D Fill 3 "<<h_2D->GetBinContent(eta2-eta1,dphi(phi1,phi2))<<endl;
      //h_2D_theta->Fill(theta2-theta1,dphi(phi2,phi1),1./N);
      //cout<<"H_2D Fill 4 "<<h_2D->GetBinContent(eta2-eta1,dphi(phi2,phi1))<<endl;
  ...
  for (int x=0;x<=h_2D->GetNbinsX();x++){
     for (int y=0;y<=h_2D->GetNbinsY();y++){
        if(h_2Dmix->GetBinContent(x,y)>0){
          ...
          //cout<<B00<<" "<<x<<" "<<y<<" "<<" "<<h_2D->GetBinContent(x,y)<<" "<<h_2Dmix->GetBinContent(x,y)<<" "<<ratio<<endl;
  ...
  fout = new TFile(Form("/mnt/c/Users/Bibek Kumar Pandit/Desktop/Root_Directory/StudyMult/VerificationCode/myoutput_minMult%d_modified1.root",mult_low),"recreate");
  fout->cd();
  h_2D->Write();
  h_2Dmix->Write();
  h_ratio->Write();
  ...
  delete fout;
    ...
    h_2D->Draw("LEGO2");
    c1->SaveAs(Form("two_plots/S%d_%d_t%d.pdf",mult_low,mult_high,isThrust));
    ...
    h_2Dmix->Draw("LEGO2");
    c2->SaveAs(Form("two_plots/B%d_%d_t%d.pdf",mult_low,mult_high,isThrust));
    ...
    h_ratio->Draw("LEGO2");
    c3->SaveAs(Form("two_plots/ratio%d_%d_t%d.pdf",mult_low,mult_high,isThrust));
    ...
    h_ratio_theta->SetTitle(Form("Ratio of correlations (in #theta) between %d and %d",mult_low,mult_high));
    h_ratio_theta->Draw("LEGO2");
    c4->SaveAs(Form("two_plots/ratio_theta%d_%d_t%d.pdf",mult_low,mult_high,isThrust));
...
void multiplot()
{
  ...
  for (int i = 0; i<3; i++)
  {
    analysis(0,1,0,low[i],high[i],40,0,3);
  }
}