...
void analysis(int isBelle = 0,int isThrust = 1, int maxevt = 0,int mult_low = 0,int mult_high = 10,int nbin = 50,bool verbose = 0,int num_runs= 1){
  ...
  filename = "/Users/anthony/desktop/LEP2MCGGUDY1997E183_mctrue_beforecut-001_thrust.aleph.root";
  ...
  //cout<<"1./nEventInMultBin"<<1./nEventInMultBin;
    ...
    for (int x=0;x<=h_2D->GetNbinsX();x++){
        ...
        for (int y=0;y<=h_2D->GetNbinsY();y++){
            ...
            if(h_2Dmix->GetBinContent(x,y)>0)
                ...
                cout<<"H2d before "<<h_2D->GetBinContent(x,y)<<endl;
                cout<<"H2dmix before"<<h_2Dmix->GetBinContent(x,y)<<endl;
    ...
    for (int x=0;x<=h_2D->GetNbinsX();x++){
     ...
     for (int y=0;y<=h_2D->GetNbinsY();y++){
       ...
       if(h_2Dmix->GetBinContent(x,y)>0){
           ...
           //cout<<"H2d after "<<h_2D->GetBinContent(x,y)<<endl;
           //cout<<"H2dmix after "<<h_2Dmix->GetBinContent(x,y)<<endl;
          ratio=B00*(h_2D->GetBinContent(x,y)/h_2Dmix->GetBinContent(x,y));
          errrel_num=h_2D->GetBinError(x,y)/h_2D->GetBinContent(x,y);
          errrel_den=h_2Dmix->GetBinError(x,y)/h_2Dmix->GetBinContent(x,y);
          errrel_ratio=TMath::Sqrt(errrel_num*errrel_num+errrel_den*errrel_den+errrel_B00*errrel_B00);
...
Double_t ftotal(Double_t *x, Double_t *par) {
   ...
   Double_t br = par[2]*background->GetBinContent(bin);
...
//   histgen();
  ...
  ftot->SetParameters(0.5*norm, 0.5*norm, 0.05);
